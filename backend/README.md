...
The IM bridge supports Feishu, Slack, Telegram, and WeChat. Slack, Telegram, and WeChat use the final `runs.wait()` response path, while Feishu streams through `runs.stream(["messages-tuple", "values"])` and updates a single in-thread card in place.
