...
def test_send_typing_fetches_ticket_then_sends_status(monkeypatch):
    from app.channels.wechat import WechatChannel
    ...
    async def go():
        ...
        def _client_factory(*args, **kwargs):
            return _MockAsyncClient(
                responses=[{"ret": 0, "typing_ticket": "ticket-1"}, {"ret": 0}],
        ...
        channel = WechatChannel(bus=MessageBus(), config={"bot_token": "bot-token", "typing_enabled": True})
        ok = await channel.send_typing("wx-user-1", "ctx-typing")

        assert ok is True
        assert len(post_calls) == 2
        assert post_calls[0]["url"].endswith("/ilink/bot/getconfig")
        assert post_calls[1]["url"].endswith("/ilink/bot/sendtyping")
        assert post_calls[1]["json"]["typing_ticket"] == "ticket-1"
        assert post_calls[1]["json"]["status"] == 1

    _run(go())


def test_chunk_reply_text_splits_long_text_into_readable_chunks():
    from app.channels.wechat import WechatChannel
    ...
    channel = WechatChannel(
        ...
        config={
            "bot_token": "bot-token",
            "chunked_reply_trigger_chars": 10,
            "chunked_reply_max_chunk_chars": 30,
            "chunked_reply_min_chunk_chars": 5,
            "chunked_reply_max_chunks": 6,
        },
    )

    chunks = channel._chunk_reply_text(
        "Sentence one. Sentence two is a bit longer. Sentence three closes."
    )

    assert chunks == [
        "Sentence one.",
        "Sentence two is a bit longer.",
        "Sentence three closes.",
    ]


def test_send_splits_long_reply_into_multiple_messages(monkeypatch):
    from app.channels.wechat import WechatChannel
    ...
    async def go():
        ...
        def _client_factory(*args, **kwargs):
            return _MockAsyncClient(responses=[{"ret": 0}, {"ret": 0}, {"ret": 0}], post_calls=post_calls, **kwargs)

        async def _no_sleep(_delay: float):
            return None

        monkeypatch.setattr("app.channels.wechat.httpx.AsyncClient", _client_factory)
        monkeypatch.setattr("app.channels.wechat.asyncio.sleep", _no_sleep)
        ...
        channel = WechatChannel(
            ...
            config={
                ...
                "chunked_reply_max_chunk_chars": 30,
                "chunked_reply_min_chunk_chars": 5,
                "chunked_reply_interval_seconds": 0,
            },
        )
        channel._context_tokens_by_chat["wx-user-1"] = "ctx-send"
        ...
        await channel.send(
            OutboundMessage(
                ...
                chat_id="wx-user-1",
                thread_id="thread-1",
                text="Sentence one. Sentence two is a bit longer. Sentence three closes.",
            ),
            _max_retries=1,
        )

        assert len(post_calls) == 3
        assert [call["json"]["msg"]["item_list"][0]["text_item"]["text"] for call in post_calls] == [
            "Sentence one.",
            "Sentence two is a bit longer.",
            "Sentence three closes.",
        ]
...
def test_send_falls_back_to_single_message_when_first_chunk_fails(monkeypatch):
    from app.channels.wechat import WechatChannel
    ...
    async def go():
        ...
        def _client_factory(*args, **kwargs):
            return _MockAsyncClient(
                post_calls=post_calls,
                post_responses=[
                    {"ret": 1, "errcode": -1, "errmsg": "chunk failed"},
                    {"ret": 0},
                ],
                **kwargs,
            )
        ...
        full_text = "Sentence one. Sentence two is a bit longer. Sentence three closes."
        await channel.send(
            OutboundMessage(
                channel_name="wechat",
                chat_id="wx-user-1",
        ...
        assert len(post_calls) == 2
        assert post_calls[0]["json"]["msg"]["item_list"][0]["text_item"]["text"] == "Sentence one."
        assert post_calls[1]["json"]["msg"]["item_list"][0]["text_item"]["text"] == full_text
...
def test_protocol_helpers_build_expected_values():
    ...
    assert int(TypingStatus.CANCEL) == 2
...
def test_handle_token_expired_rebinds_when_enabled(monkeypatch, tmp_path: Path):
    from app.channels.wechat import WechatChannel
    ...
    async def go():
        ...
        def _client_factory(*args, **kwargs):
            return _MockAsyncClient(
                get_calls=get_calls,
                get_responses=[
                    {"qrcode": "qr-456", "qrcode_img_content": "https://example.com/qr2.png"},
                    {"status": "confirmed", "bot_token": "renewed-token", "ilink_bot_id": "bot-2"},
                ],
                **kwargs,
            )
        ...
        channel = WechatChannel(
            ...
            config={
                "state_dir": str(state_dir),
                "qrcode_login_enabled": True,
                "auto_rebind_on_expired": True,
                "qrcode_poll_interval": 0.01,
                "qrcode_poll_timeout": 1,
            },
        )
        channel._bot_token = "expired-token"
        channel._get_updates_buf = "cursor-old"

        ok = await channel._handle_token_expired()

        assert ok is True
        assert channel._bot_token == "renewed-token"
